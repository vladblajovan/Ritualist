...
/// Schema V4: Replaced isPinned with notes property in HabitModel
///
/// Changes from V3:
/// - HabitModel: Removed `isPinned: Bool` property
...
public enum SchemaV4: VersionedSchema {
    ...
    public final class HabitModel {
        ...
        public var notes: String?  // NEW in V4: Optional notes for habit
        // isPinned property REMOVED in V4

        // MARK: - Relationships
        @Relationship(deleteRule: .cascade, inverse: \HabitLogModel.habit)
        public var logs: [HabitLogModel] = []

        public var category: SchemaV4.HabitCategoryModel?
...
public typealias HabitModelV4 = SchemaV4.HabitModel
public typealias HabitLogModelV4 = SchemaV4.HabitLogModel
public typealias HabitCategoryModelV4 = SchemaV4.HabitCategoryModel
public typealias UserProfileModelV4 = SchemaV4.UserProfileModel
public typealias OnboardingStateModelV4 = SchemaV4.OnboardingStateModel
public typealias PersonalityAnalysisModelV4 = SchemaV4.PersonalityAnalysisModel

// MARK: - Domain Entity Conversions

/// Extensions to convert between SchemaV4 models and domain entities

extension SchemaV4.HabitModel {
    ...
    public static func fromEntity(_ habit: Habit, context: ModelContext? = nil) throws -> HabitModelV4 {
        ...
        var category: SchemaV4.HabitCategoryModel?
        if let categoryId = habit.categoryId, let context = context {
            let descriptor = FetchDescriptor<SchemaV4.HabitCategoryModel>(predicate: #Predicate { $0.id == categoryId })
            category = try? context.fetch(descriptor).first
        }

        return SchemaV4.HabitModel(
            id: habit.id,
            name: habit.name,
            ...
            // isPinned property ignored (removed in V4)
        )
    }
}

extension SchemaV4.HabitLogModel {
    /// Convert SwiftData model to domain entity
    ...
    /// Create SwiftData model from domain entity
    public static func fromEntity(_ log: HabitLog, context: ModelContext? = nil) -> HabitLogModelV4 {
        var habit: HabitModelV4?
        if let context = context {
            let descriptor = FetchDescriptor<SchemaV4.HabitModel>(predicate: #Predicate { $0.id == log.habitID })
            habit = try? context.fetch(descriptor).first
        }
        return SchemaV4.HabitLogModel(id: log.id, habitID: log.habitID, habit: habit, date: log.date, value: log.value, timezone: log.timezone)
    }
}

extension SchemaV4.HabitCategoryModel {
    /// Convert SwiftData model to domain entity
    ...
    /// Create SwiftData model from domain entity
    public static func fromEntity(_ category: HabitCategory) -> HabitCategoryModelV4 {
        HabitCategoryModelV4(
            id: category.id,
            name: category.name,
...
extension SchemaV4.UserProfileModel {
    ...
    public func toEntity() -> UserProfile {
        ...
        return UserProfile(
            ...
            timezoneChangeHistory: [],  // No history in V4
            createdAt: createdAt,
            updatedAt: updatedAt
        )
    }
    ...
    /// Create SwiftData model from domain entity
    public static func fromEntity(_ profile: UserProfile) -> UserProfileModelV4 {
        // Convert V9 three-timezone model back to V4 format
        let homeTimezoneV4 = profile.homeTimezoneIdentifier
        let displayModeV4 = profile.displayTimezoneMode.toLegacyString()

        return SchemaV4.UserProfileModel(
            id: profile.id.uuidString,
            name: profile.name,
            avatarImageData: profile.avatarImageData,
            appearance: String(profile.appearance),
            homeTimezone: homeTimezoneV4,
            displayTimezoneMode: displayModeV4,
...
extension SchemaV4.OnboardingStateModel {
    /// Convert SwiftData model to domain entity
    ...
    /// Create SwiftData model from domain entity
    public static func fromEntity(_ state: OnboardingState) -> OnboardingStateModelV4 {
        return SchemaV4.OnboardingStateModel(
...
extension SchemaV4.PersonalityAnalysisModel {
    /// Convert SwiftData model to domain entity
    ...
    /// Create SwiftData model from domain entity
    public static func fromEntity(_ entity: PersonalityProfile) -> PersonalityAnalysisModelV4 {
        PersonalityAnalysisModelV4(
