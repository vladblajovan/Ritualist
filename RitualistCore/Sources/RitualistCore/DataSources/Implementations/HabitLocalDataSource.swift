...
public actor HabitLocalDataSource: HabitLocalDataSourceProtocol {
    ...
    public func fetchAll() async throws -> [Habit] {
        do {
            let descriptor = FetchDescriptor<HabitModelV1>(
    ...
    public func upsert(_ habit: Habit) async throws {
        do {
            ...
            if let categoryId = habit.categoryId {
                let categoryDescriptor = FetchDescriptor<HabitCategoryModelV1>(predicate: #Predicate { $0.id == categoryId })
            ...
            let habitModel = try HabitModelV1.fromEntity(habit, context: modelContext)
    ...
    public func delete(id: UUID) async throws {
        do {
            let descriptor = FetchDescriptor<HabitModelV1>(predicate: #Predicate { $0.id == id })
            let habits = try modelContext.fetch(descriptor)
            ...
            let verifyDescriptor = FetchDescriptor<HabitModelV1>(predicate: #Predicate { $0.id == id })
    ...
    public func cleanupOrphanedHabits() async throws -> Int {
        do {
            // Get all habits
            let habitDescriptor = FetchDescriptor<HabitModelV1>()
            let allHabits = try modelContext.fetch(habitDescriptor)

            // Get all existing category IDs
            let categoryDescriptor = FetchDescriptor<HabitCategoryModelV1>()
