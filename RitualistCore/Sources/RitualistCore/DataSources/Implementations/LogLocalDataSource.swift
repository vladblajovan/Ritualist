...
public actor LogLocalDataSource: LogLocalDataSourceProtocol {
    ...
    public func logs(for habitID: UUID) async throws -> [HabitLog] {
        let descriptor = FetchDescriptor<HabitLogModelV1>(
    ...
    public func logs(for habitIDs: [UUID]) async throws -> [HabitLog] {
        ...
        let descriptor = FetchDescriptor<HabitLogModelV1>(
            predicate: #Predicate<HabitLogModelV1> { habitIDs.contains($0.habitID) },
            sortBy: [SortDescriptor(\.date, order: .reverse)]
        )
        let logs = try modelContext.fetch(descriptor)
        return logs.map { $0.toEntity() }
    ...
    public func upsert(_ log: HabitLog) async throws {
        ...
        if let existing = try modelContext.fetch(descriptor).first {
            ...
            let habitLogModel = HabitLogModelV1.fromEntity(log, context: modelContext)
    ...
    public func delete(id: UUID) async throws {
        let descriptor = FetchDescriptor<HabitLogModelV1>(predicate: #Predicate { $0.id == id })
        let logs = try modelContext.fetch(descriptor)
