...
public protocol StreakEngine {
    func currentStreak(for habit: Habit, logs: [HabitLog], asOf: Date) -> Int
    func bestStreak(for habit: Habit, logs: [HabitLog]) -> Int
}

public final class DefaultStreakEngine: StreakEngine {
    private let dateProvider: DateProvider
    public init(dateProvider: DateProvider) { self.dateProvider = dateProvider }

    public func currentStreak(for habit: Habit, logs: [HabitLog], asOf: Date) -> Int {
    ...
    private func calculateWeeklyStreak(for habit: Habit, logs: [HabitLog], target: Int, asOf: Date) -> Int {
        ...
        var streak = 0
        var date = dateProvider.startOfDay(asOf)
    ...
    public func bestStreak(for habit: Habit, logs: [HabitLog]) -> Int {
        ...
        let compliantDates = Set(logs.compactMap { log in
            ...
            return valueOK ? dateProvider.startOfDay(log.date) : nil
        })
        // Sort ascending
