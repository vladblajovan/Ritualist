...
public actor StoreKitSubscriptionService: SecureSubscriptionService {
    ...
    public func registerPurchase(_ productId: String) async throws {
        cachedValidPurchases.insert(productId); lastCacheUpdate = Date()
        ...
        if !transactionInfo.found {
            Container.shared.debugLogger().log("Transaction not found after retries for \(productId) - using fallback expiry", level: .warning, category: .subscription)
            expiryDate = Self.fallbackExpiryDate(for: plan)
        }
        await SecurePremiumCache.shared.updateCache(plan: plan, isOnTrial: transactionInfo.isOnTrial, expiryDate: expiryDate)
    ...
    private func queryTransactionWithRetry(productId: String) async -> TransactionQueryResult {
        ...
        for attempt in 1...3 {
            var latestPurchaseDate: Date?; var result = TransactionQueryResult()
            for await entitlement in Transaction.currentEntitlements {
                guard case .verified(let transaction) = entitlement, transaction.productID == productId else { continue }
                let shouldUpdate = latestPurchaseDate.map { transaction.purchaseDate > $0 } ?? true
                if shouldUpdate { latestPurchaseDate = transaction.purchaseDate; result.isOnTrial = transaction.offer?.type == .introductory; result.expiryDate = transaction.expirationDate }
            }
            if latestPurchaseDate != nil { result.found = true; return result }
            if attempt < 3 { try? await Task.sleep(nanoseconds: delayNanoseconds); delayNanoseconds *= 2 }
        }
    ...
    public func clearPurchases() async throws {
        cachedValidPurchases.removeAll(); lastCacheUpdate = .distantPast
    ...
    public func getCurrentSubscriptionPlan() async -> SubscriptionPlan {
        ...
        if planFromMemory != .free { return planFromMemory }
    ...
    public func getSubscriptionExpiryDate() async -> Date? {
        ...
        for await result in Transaction.currentEntitlements {
            ...
            if transaction.productID.contains("monthly") || transaction.productID.contains("annual"), let expirationDate = transaction.expirationDate { return expirationDate }
        }
        return nil
    }
    ...
    public func isOnTrial() async -> Bool {
        for await result in Transaction.currentEntitlements {
            guard let transaction = try? checkVerified(result) else { continue }
            if transaction.offer?.type == .introductory { return true }
        }
        return false
    }
    ...
    private func refreshCacheIfNeeded() async { if Date().timeIntervalSince(lastCacheUpdate) > cacheValidityDuration { await refreshCache(force: false) } }
    ...
    private func refreshCache(force: Bool) async {
        ...
        cachedValidPurchases = entitlementResult.validPurchases; lastCacheUpdate = Date()
        await updateSecureCache(with: entitlementResult)
    }

    private func shouldRefreshCache(force: Bool) -> Bool { force || Date().timeIntervalSince(lastCacheUpdate) > cacheValidityDuration }

    private struct TransactionQueryResult { var found = false; var isOnTrial = false; var expiryDate: Date? }
    private struct EntitlementResult { var validPurchases: Set<String> = []; var didReceiveAnyEntitlement = false; var isOnTrial = false; var expiryDate: Date? }
    ...
    private func processCurrentEntitlements() async -> EntitlementResult {
        ...
        for await verificationResult in Transaction.currentEntitlements {
            ...
            if transaction.offer?.type == .introductory { result.isOnTrial = true }
            if let transactionExpiry = transaction.expirationDate {
                if let currentExpiry = result.expiryDate { if transactionExpiry < currentExpiry { result.expiryDate = transactionExpiry } } else { result.expiryDate = transactionExpiry }
            }
        }
        return result
    }
    ...
    private func updateSecureCache(with result: EntitlementResult) async {
        ...
        if plan != .free {
            let effectiveExpiryDate: Date? = result.expiryDate ?? {
                Container.shared.debugLogger().log("Active subscription (\(plan)) has no expiry date - using 1-day fallback for cache", level: .warning, category: .subscription)
                return Calendar.current.date(byAdding: .day, value: 1, to: Date())
            }()
            await SecurePremiumCache.shared.updateCache(plan: plan, isOnTrial: result.isOnTrial, expiryDate: effectiveExpiryDate)
        } else if result.didReceiveAnyEntitlement {
            await SecurePremiumCache.shared.updateCache(plan: .free)
        } else {
            if !(await SecurePremiumCache.shared.isCacheValid()) { await SecurePremiumCache.shared.updateCache(plan: .free) }
        }
    }

    private func subscriptionPlanFromPurchases(_ purchases: Set<String>) -> SubscriptionPlan {
        if purchases.contains(StoreKitProductID.annual) { return .annual }
        if purchases.contains(StoreKitProductID.monthly) { return .monthly }
        if purchases.contains(StoreKitProductID.weekly) { return .weekly }
        return .free
    }
    ...
    private func isUserInGracePeriod() async -> Bool {
        if await SecurePremiumCache.shared.shouldSuppressBillingDialog() {
            Self.startupLogger.log("🔔 Billing dialog suppressed (24h) - assuming grace period active", level: .info, category: .subscription); return true
        }
        do {
            let statuses = try await Product.SubscriptionInfo.status(for: StoreKitProductID.subscriptionGroupID)
            for status in statuses { if let result = await handleGracePeriodState(status.state) { return result } }
            return false
        } catch {
            Self.startupLogger.log("⚠️ Failed to check subscription status for grace period: \(error.localizedDescription)", level: .warning, category: .subscription); return false
        }
    }

    private func handleGracePeriodState(_ state: StoreKit.Product.SubscriptionInfo.RenewalState) async -> Bool? {
        switch state {
        case .subscribed: await SecurePremiumCache.shared.clearBillingIssueFlag(); return true
        case .inGracePeriod: Self.startupLogger.log("🔔 User in billing grace period - retaining premium access", level: .info, category: .subscription); await SecurePremiumCache.shared.recordBillingIssueDetected(); return true
        case .inBillingRetryPeriod: Self.startupLogger.log("🔔 User in billing retry period - retaining premium access", level: .info, category: .subscription); await SecurePremiumCache.shared.recordBillingIssueDetected(); return true
        case .expired: Self.startupLogger.log("🔔 Subscription expired - clearing all caches", level: .info, category: .subscription); await SecurePremiumCache.shared.clearCache(); return false
        case .revoked: Self.startupLogger.log("🔔 Subscription revoked - clearing all caches", level: .info, category: .subscription); await SecurePremiumCache.shared.clearCache(); return false
        default: return nil
        }
    }
    ...
    private func checkVerified<T>(_ result: VerificationResult<T>) throws -> T? {
        ...
        case .unverified(_, let verificationError):
            Task { await errorHandler?.logError(PaywallError.purchaseFailed("Transaction verification failed: \(verificationError)"), context: ErrorContext.userInterface + "_transaction_verification", additionalProperties: ["error": "\(verificationError)"]) }
            return nil
