...
public actor OnboardingLocalDataSource: OnboardingLocalDataSourceProtocol {
    ...
    public func load() async throws -> OnboardingState? {
        ...
        guard let sdState = try modelContext.fetch(descriptor).first else {
            return nil
        }
        return OnboardingMapper.fromSD(sdState)
    }
    ...
    public func save(_ state: OnboardingState) async throws {
        ...
        if let existing = try modelContext.fetch(descriptor).first {
            ...
            let sdState = OnboardingMapper.toSD(state)
            modelContext.insert(sdState)
        }
