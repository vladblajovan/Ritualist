...
public actor CategoryLocalDataSource: CategoryLocalDataSourceProtocol {
    ...
    public func updateCategory(_ category: Category) async throws {
        ...
        if let existingSDCategory = try modelContext.fetch(descriptor).first {
            // Update existing category
            existingSDCategory.name = category.name
            existingSDCategory.displayName = category.displayName
            existingSDCategory.emoji = category.emoji
            existingSDCategory.order = category.order
            existingSDCategory.isActive = category.isActive
            existingSDCategory.isPredefined = category.isPredefined
            try modelContext.save()
        } else {
